...
void ResourceManager::loadModel(std::string name, std::string path, glm::mat4 transform, bool flipTexture)
{
	...
	model.vertexBuffer   = std::make_unique<Buffer>(device, loader.vertices, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | rayTracingFlags);
	model.indexBuffer    = std::make_unique<Buffer>(device, loader.indices, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | rayTracingFlags);
	model.matColorBuffer = std::make_unique<Buffer>(device, loader.materials, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | flag);
	model.matIndexBuffer = std::make_unique<Buffer>(device, loader.matIndx, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | flag);

	auto txtOffset = static_cast<uint32_t>(textures.size());
	createTextureImages(loader.textures,flipTexture);
	...
	objModel.emplace_back(model);
	objDesc.emplace_back(desc);

}

void ResourceManager::createTextureImages(const std::vector<std::string>& textures,bool flipTexture)
{
	for(const auto& textureStr : textures)
	{
		std::unique_ptr<Image> image = std::make_unique<Image>(device,textureStr,flipTexture);
		std::unique_ptr<ImageView> imageView = std::make_unique<ImageView>(*image);
		
		Texture texture;
		texture.image     = image->getHandle();
		texture.imageView = imageView->getHandle();
		texture.sampler   = defaultSampler->getHandle();
